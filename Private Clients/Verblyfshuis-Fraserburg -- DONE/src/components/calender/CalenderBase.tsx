...
import { generateArr } from "../../services/utils/Utilities";
...
export class BaseCalender extends Component<iProps, iState> {
  ...
  generateEntries(startNum: number, endNum: number, isRev: boolean) {
    ...
    for (let i = 0; i < endNum; i++) {
      returnArr.push(
        <IonButton className='dayButton' color='primary' onClick={() => this.onDayClick(`notCurr ${i + 1}`)} fill='clear' disabled={isRev ? true : false}>
          {use[i]}
        </IonButton>
      );
    }
            ...
            for (let i = 0; i < needed; i++) {
              ...
              temp.push(
                <IonCol className='calRow' key={`end ${ends.length + i + 1}`} id={`end ${ends.length + i + 1}`}>
                  <IonButton className='dayButton' color='primary' onClick={() => this.onDayClick(`notCurr ${i + 1}`)} fill='clear' disabled={false}>
                    {ends.length + i + 1}
                  </IonButton>
                </IonCol>
              );
