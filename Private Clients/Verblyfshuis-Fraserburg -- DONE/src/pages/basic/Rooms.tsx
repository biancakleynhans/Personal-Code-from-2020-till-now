...
const Text =
  'The main bedroom has a double bed and a small lounge, and the second bedroom two single beds. The kitchen, with gas stove, cooking utensils and fridge, is open plan with the lounge. The bathroom has a walk-in shower (water is heated by a solar geyser), Washbasin and toilet. Solar power provides electricity. Wifi and TV are not available although wifi can be accessed at our house, a three minute walk from the Verblyfhuis.';

const textForDetails1 = ['Self-catering', 'Organic meals available by prior arrangement'];
const textForDetails2 = ['Open plan', 'Breathtaking views'];
const textForDetails3 = ['Scenic', 'Hiking'];
        ...
        <IonCard style={{ width: '675px' }}>
          <IonCardHeader>
            <IonCardTitle style={{ fontSize: '28px', textAlign: 'center' }}>
              <IonText color='primary'>Verblyfhuis</IonText>
            </IonCardTitle>
          </IonCardHeader>
          <IonCardContent>
            <IonItem lines='full'>
    ...
    return (
      <IonRow ref={introRef} className='ion-align-self-center'>
        <IonCard>
          ...
          <IonCardContent>
            {[...textForDetails1, ...textForDetails2, ...textForDetails3].map((entry) => {
              return (
                <IonItem lines='full'>
                  <IonIcon color='primary' icon={leafOutline} />
                  <IonLabel class="class='ion-text-wrap">{entry}</IonLabel>
